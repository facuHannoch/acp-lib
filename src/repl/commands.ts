// Reusable slash-command handlers for interactive sessions.
...
export interface InteractiveAgentClient extends AgentClient, Bridgeable {
  ...
  setMode(mode: AgentMode): Promise<ConnectResult>;
  parseConfigValue(
    configId: string,
    rawValue: string,
  ): ConfigValueParseResult;
...
export interface CommandHandlerDeps {
  /** Prompt client plus the interactive control-plane operations used by commands. */
...
export function createAgentCommands(deps: CommandHandlerDeps): AgentCommands {
  ...
  const onSlashCommand = async (command: string, commandArgs: string[]): Promise<boolean> => {
    switch (command) {
      case "help": {
        note(
          `commands: /help ${client.getCommands().map((c) => c.usage).join(" ")} ` +
